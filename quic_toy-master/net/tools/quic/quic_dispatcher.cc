...
class QuicDispatcher::QuicFramerVisitor : public QuicFramerVisitorInterface {
 ...
 bool OnStreamFrame(const QuicStreamFrame& /*frame*/) override {
  ...
  bool OnPathCloseFrame(const QuicPathCloseFrame& frame) { 
    DCHECK(false); 
    return false;
  }  
...
QuicDispatcher::QuicDispatcher(const QuicConfig& config,
                               ...
                               /*QuicPacketWriterFactory* packet_writer_factory,*/
...
QuicServerSession* QuicDispatcher::CreateQuicSession(
  ...
  QuicConnection* connection = new QuicConnection(
      connection_id, client_address, helper_.get(), alarm_factory_.get(), writer_.get(),
  ...
  session->InitializeSession(crypto_config_, &compressed_certs_cache_, session_helper_.get());
  return session;
}

QuicTimeWaitListManager* QuicDispatcher::CreateQuicTimeWaitListManager() {
  return new QuicTimeWaitListManager(
      writer_.get(), this, helper_.get(), alarm_factory_.get(), supported_versions());
}
