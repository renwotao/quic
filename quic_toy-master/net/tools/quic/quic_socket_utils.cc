  ...
  if (hdr->msg_controllen > 0) {
    for (cmsghdr* cmsg = CMSG_FIRSTHDR(hdr); cmsg != nullptr;
      ...
      if (cmsg->cmsg_type == IPV6_PKTINFO) {
        ...
        addr_data = reinterpret_cast<uint8_t*>(&info->ipi6_addr);
        len = sizeof(in6_addr);
      } else if (cmsg->cmsg_type == IP_PKTINFO) {
        in_pktinfo* info = reinterpret_cast<in_pktinfo*>CMSG_DATA(cmsg);
        addr_data = reinterpret_cast<uint8_t*>(&info->ipi_addr);
        len = sizeof(in_addr);
      } else {
	continue;
      }
...
size_t QuicSocketUtils::SetIpInfoInCmsg(const IPAddress& self_address,
  ...
  if (GetAddressFamily(self_address) == ADDRESS_FAMILY_IPV4) {
    ...
    memcpy(&pktinfo->ipi_spec_dst, self_address.bytes().data(), self_address.size());
    ...
    memcpy(&pktinfo->ipi6_addr, self_address.bytes().data(), self_address.size());
