...
void QuicTimeWaitListManager::ProcessPacket(
  ...
  if (it->second.termination_packets.empty()) {
    if (it->second.connection_rejected_statelessly) {
      DVLOG(3) << "Time wait list sending previous statlessley reject response "
               << "for connection " << connection_id;
    }
    for (const auto& packet : it->second.termination_packets) {
...
// owner of the packet.
void QuicTimeWaitListManager::SendOrQueuePacket(std::unique_ptr<QueuedPacket> packet) {
